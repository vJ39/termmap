...
//! ブラウザ側(web/touch-overlay.js)に対応表を二重に持たせないため判定は Rust 側へ集約し、結果だけを
//! OSC 9997 で通知する(sound.rs の 9999・voice.rs の 9998 と同じで、認識しない端末では無視される)。
...
/// ブラウザ → termmap のパン量マーカーの先頭(設計書 §6.1)。形式は `\u{1}PAN\u{1}<fx>\u{1}<fy>\u{1}`。
/// fx/fy は指の移動量を端末ビューポートの幅/高さで割った比で、符号は指の移動方向そのもの(右/下が正)。
/// ライブ現在地の `\u{1}GPS\u{1}...` と同じ SOH 区切りの専用マーカーで、通常のペーストとは衝突しない。
...
/// パン量マーカー(`\u{1}PAN\u{1}<fx>\u{1}<fy>\u{1}`)を (fx, fy) に解く。壊れた入力は None を返して
/// 無視する(`src/ui.rs` の `\u{1}GPS\u{1}` 分岐と同じ扱い)。範囲外(|比| > 1)は1回の touchmove で端末
/// 1画面分を超えて動いたことになり実機ではありえないので、クランプせずマーカーごと捨てる(壊れた値を
/// 丸めて画面が飛ぶより、その1回分を落とす方が被害が小さい)。
...
/// 指の移動比 → 地図の移動量[出力ピクセル]の換算(設計書 §6.2)。「指が端末の1/4を横切ったら地図も
/// 表示範囲の1/4動く」を式で保証する。端末の桁数/行数(`cols`/`rows`)と地図領域(`map_cols`/`map_rows`・
/// `ow`/`oh`)の両方を Rust が持つので、左袖やステータス行のぶんもここで吸収できる(JS 側にゲイン調整の
/// 定数を置かずに済む)。返すのは符号を反転していない生の移動量で、反転(`cx -= dx`)は呼び出し側で行う。
...
/// `Axis::Pan` の軸だけを X/Y 独立に動かし(`PoiList` は X だけ)、中心は指と逆向きに動かす(設計書 §3.1)。
/// X は `rem_euclid` で巻き、Y は端でクランプする(合算値は1マーカーの上限±1を超えうるので1回ぶんの加減算
/// では足りない)。戻り値は (新しいcx, 新しいcy, 実際に動かしたか)。動かさなければ入力をそのまま返す。
