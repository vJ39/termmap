// 地図に重ねるプロットデータのディスク層(ネットワークにもデータ型にも触れない)。キー1件=1ファイルの
// JSON を ~/.config/termmap/plot-cache/v1/<種別>/<キー>.json に保存し、種別ごとの差(ディレクトリ名・
// TTL・上限)は Layer に集約する。設計は docs/plot-data-disk-cache-design.md §3/§5/§8、境界は
// docs/disaster-choropleth-design.md §2.7、人口は docs/population-mesh-overlay-design.md。
// 期限判定に mtime を使わないのは、stale でも中身を表示するのでどのみちファイルを読むため(tiles.rs と
// 違う点)。mtime は gc() がファイルを開かずに古い順へ並べるためだけに使う。
...
impl Layer {
    ...
    /// この期間内なら再取得しない(通信を止める唯一の判断基準)。交通量5分=元データが5分値なので、
    /// 短く叩いても新しい値は無い。通行規制10分=数十分〜数日で変わり、通行止めは安全に直結する。
    /// カメラ7日=新設・撤去は週〜月単位。主要道路30日(tiles.rs のタイルTTLと同じ)・過去災害30日=
    /// 月単位でしか変わらない。市区町村境界180日=合併等でごく稀。人口メッシュ365日=改定は数年に1度。
    pub fn fresh_ttl(&self) -> Duration {
        Duration::from_secs(match self {
            ...
            // 人口メッシュ365日=推計の改定は数年に1度(前版が令和2年国勢調査ベース)。
    ...
    /// 通行規制24時間=災害・工事の通行止めは日単位で続くので「N時間前時点の規制」として出す価値がある。
    /// 道路・カメラ・過去災害・市区町村境界は上限なし(古くても誤りにならず、新しいものが出ないだけ)。
