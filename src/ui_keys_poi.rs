// 探して候補を出す画面のキー処理(ui_keys.rs の Focus 分岐から切り出した1つ)。地名検索(/)・道路名検索・
// AIおすすめ・目的地カテゴリ(f)・キーワード周辺検索・その結果一覧。どれも問い合わせを別スレッドへ投げ
// (結果は ui_jobs 側で受け取る)、待っている間も地図を動かせるよう画面は Map へ戻す。
...
mod tests {
    ...
    // ままにする」分岐だけが focus を書き戻す)。問い合わせを実際に投げる分岐(通信スレッドを起こす)と、
    // 目的地カテゴリの並べ替え/削除/追加($HOME/.config/termmap/poi-kinds.txt を書く)はテストから触らない。
    // ここで確かめるのは入力の受け付け方・カーソル移動・画面遷移・キャッシュヒット時の適用だけ。
    fn base() -> UiState {
