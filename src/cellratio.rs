//! 端末セル比(r = セル高 / セル幅)の取得と、写真を歪ませずにセル矩形へ収める当てはめ計算(純関数)。
//! 背景は docs/web-image-aspect-ratio-design.md。実画像モード(OSC 1337)は `preserveAspectRatio=0` で
//! セル矩形へ強制フィットするので、縦横比が食い違うとそのぶん歪む(設計書 §4.1)。写真は中央から
//! 切り出す(cover方式。設計書 §7.6 のレターボックス案は写真が小さく見えるためクロップへ変更)。
//! 地図タイル側の歪み(設計書 §2)は扱わない。`src/ui.rs` の `rh` に r を掛けると、`oh` が `map_rows * 2`
//! のままの `dragmode::pan_ratio_to_px` とパン量換算がズレるので、`Layout` の見直しとセットで別途行う。
...
/// 写真をセル矩形の縦横比で中央から切り出す(cover方式)ときの、切り出し矩形。出力画素の縦横比を
/// セル矩形の物理的な縦横比に合わせるので、`preserveAspectRatio=0` で強制フィットさせても歪まない。
/// 余白(黒帯)は生じない代わりに、セル矩形からはみ出す方向の端は見切れる。
...
/// 写真(`img_w` x `img_h`)から `cols` x `rows` セルの矩形と同じ縦横比の領域を、中央を基準に
/// 切り出す(cover方式、設計書 §7.6 のレターボックス案から変更)。セル1個は 幅1 : 高さ r なので、
/// 矩形の物理的な縦横比は `cols : rows*r`。写真は拡大せず、矩形の方が横長なら上下を、縦長なら左右を
/// 切り落とす。計算できない入力(0寸法・非有限/非正の r)では写真全体を返す(挙動が悪化しない)。
...
mod tests {
    ...
    fn parse_cell_marker_reads_what_the_overlay_actually_sends() {
        // web/touch-overlay.js の sendCellSize() が組み立てる文字列(先頭SOH + 小数4桁)そのもの。
        // JS 側とズレると、セル比が届かず既定値のまま(写真が歪んだまま)静かに壊れるので固定しておく。
