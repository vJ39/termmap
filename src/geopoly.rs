// 多角形の純粋な幾何(内外判定と外接矩形)。std だけに依存し crate:: を参照しない。設計は
// docs/disaster-choropleth-design.md §5.2。muni.rs から分けてあるのは、気象警報が class10s の
// 区域に対して同じ内外判定を必要とするため。座標系は持たず、termmap は (緯度, 経度) で呼ぶので
// rings_bbox の戻り値は plotlayer::Bbox と同じ (lat_min, lon_min, lat_max, lon_max) の並びになる。
// リング列は外周・穴・離島を区別せず平らに受ける(even-odd なら1回の走査で判定できる)。class20s は
// リングの巻き方向が不統一(CW/CCW が混在)なので、向きに依存する nonzero winding は使えない。

/// 点がリング列の内側かを even-odd(レイキャスティング)で判定する。辺の採否は
/// `(b_i > pb) != (b_j > pb)` の半開区間規則にし、頂点を通る走査線で交点を二重に数えて内外が
/// 裏返るのを防ぐ。境界線上の点は「第1成分が小さい側の辺は内側・大きい側の辺は外側」になり、
/// 隣り合う区域で二重に当たらない。頂点が2つ以下のリングは面を持たないので無視し、空配列なら false。
