...
// idx は Focus::Settings 側の項目行番号と対応(4=地図種別/5=既定ルート/9=提案AIモデル/12=画像解像度/18=QR表示方式/20=雨雲の濃さ/30=人口の濃さ)。
// 中心十字の色(idx=16)は cfg.cross_color_idx が String でなく u8 なので、この表とは別枠(is_pickable等で16を特別扱い)。
// 人口の年次(idx=31)も cfg.population_year が u16 なので同じく別枠。
...
pub(crate) fn is_pickable(idx: usize) -> bool { idx == 16 || idx == 27 || idx == 31 || choice_for(idx).is_some() }

// 人口メッシュの年次(idx=31)の候補。2020年は令和2年国勢調査に基づく実績で、それ以降は推計値。
...
pub(crate) fn pick_current(idx: usize, cfg: &Config, style: &str) -> usize {
    match idx {
        ...
        30 => choice_for(30).and_then(|c| c.values.iter().position(|v| *v == cfg.population_opacity)).unwrap_or(0),
        31 => crate::config::POPULATION_YEARS.iter().position(|y| *y == cfg.population_year).unwrap_or(0),
        _ => 0,
    }
}
...
pub(crate) fn apply_pick(idx: usize, sel: usize, cfg: &mut Config, style: &mut String) -> ApplyEffect {
    ...
    match idx {
        ...
        30 => if let Some(v) = choice_for(30).and_then(|c| c.values.get(sel)) { cfg.population_opacity = v.to_string(); eff.force_reemit = true; }
        31 => if let Some(y) = crate::config::POPULATION_YEARS.get(sel) { cfg.population_year = *y; eff.force_reemit = true; }
        _ => {}
    }
    eff
}
...
pub(crate) fn setting_description(idx: usize) -> &'static str {
    match idx {
        ...
        29 => "人口メッシュ: 500mメッシュごとの推計人口(国土数値情報)を人口密度の濃さで地図に重ねる。補給・宿・明かりが期待できる帯と、人がいない帯が読める(Uキーでも切替)。1都道府県が最大31MBあり、取得は都道府県まるごと・数十秒かかる。一度取れば1年は取り直さない。ONにした人だけが外部サービスへ問い合わせる",
        30 => "人口の濃さ: 重ねる強さ。Enterで一覧を開いて選択(薄い=地図優先 / 標準 / 濃い=人口優先)。人口の少ない土地は元から薄く塗るので、濃くしても地図は残る",
        31 => "人口の年次: 表示する年。Enterで一覧を開いて選択。2020年は国勢調査に基づく実績、2025年以降は推計値。年を変えても取り直しは起きない(全年次をまとめて保存している)",
...
pub(crate) fn settings_rows(opts: &Args, cfg: &Config, picking: Option<usize>, onboarded: bool, set_sel: usize, set_pick_sel: usize) -> (String, Vec<String>, usize) {
    ...
    let mut its = vec![
        ...
        format!("{} 人口の濃さ {}", arrow(30), match cfg.population_opacity.as_str() { "light" => "薄い", "strong" => "濃い", _ => "標準" }),
        format!("{} 人口の年次 {}年", arrow(31), cfg.population_year),
    ];
...
mod tests {
    ...
    fn pickable_covers_the_multi_choice_items_and_cross_color() {
        for idx in [4usize, 5, 9, 12, 16, 18, 20, 27, 30, 31] {
    ...
    fn pick_labels_len_matches_choice_values_len() {
        ...
        for idx in [4usize, 5, 9, 12, 18, 20, 30] {
        ...
        assert_eq!(pick_labels(31, &cfg).len(), crate::config::POPULATION_YEARS.len());
    }
    ...
    fn setting_description_covers_every_known_row_distinctly() {
        // 0〜16,18〜31 は個別の説明文を持つ(idx=11/27は端末対応有無で文言が変わるが、いずれにせよ
        // 空でない。17=Google APIキーはフォールバック経由で別テストで確認するためここでは含めない)。
        let mut seen = Vec::new();
        for idx in (0usize..=16).chain(18..=31) {
    ...
    fn settings_rows_end_with_the_three_population_rows() {
        ...
        assert_eq!(its.len(), SETTINGS_ROW_COUNT);
        assert_eq!(its[29], "人口メッシュ ON");
        assert_eq!(its[30], "▸ 人口の濃さ 濃い");
        assert_eq!(its[31], "▸ 人口の年次 2050年");
    ...
    fn the_population_rows_default_to_off_mid_and_2025() {
        let cfg = Config::default();
        let (_, its, _) = settings_rows(&test_args(), &cfg, None, false, 0, 0);
        assert_eq!(its[29], "人口メッシュ OFF", "既定OFF(1都道府県が最大31MBあるため)");
        assert_eq!(its[30], "▸ 人口の濃さ 標準");
        assert_eq!(its[31], "▸ 人口の年次 2025年");
    }
    ...
    fn pick_current_and_apply_pick_roundtrip_population_opacity() {
        let mut cfg = Config::default();
        let mut style = "osm".to_string();
        assert_eq!(pick_current(30, &cfg, &style), 1); // 既定 "mid"
        let eff = apply_pick(30, 2, &mut cfg, &mut style);
        assert_eq!(cfg.population_opacity, "strong");
        assert!(eff.force_reemit, "濃さを変えたら即描き直す");
        assert!(!eff.cache_clear);
        assert_eq!(pick_current(30, &cfg, &style), 2);
        // 範囲外の選択は無視する。
        let before = cfg.population_opacity.clone();
        let eff2 = apply_pick(30, 99, &mut cfg, &mut style);
    ...
    fn pick_current_and_apply_pick_roundtrip_population_year() {
        let mut cfg = Config::default();
        let mut style = "osm".to_string();
        assert_eq!(pick_current(31, &cfg, &style), 1, "既定2025は2番目(先頭は2020)");
        let eff = apply_pick(31, 0, &mut cfg, &mut style);
        assert_eq!(cfg.population_year, 2020);
        assert!(eff.force_reemit);
        assert_eq!(pick_current(31, &cfg, &style), 0);
        let eff2 = apply_pick(31, 10, &mut cfg, &mut style);
        assert_eq!(cfg.population_year, 2070);
        assert!(eff2.force_reemit);
        // 範囲外の選択は無視する。
        let eff3 = apply_pick(31, 99, &mut cfg, &mut style);
        assert_eq!(cfg.population_year, 2070);
    ...
    fn pick_current_unknown_population_values_default_to_zero() {
        ...
        assert_eq!(pick_current(30, &cfg, "osm"), 0);
    ...
    fn setting_description_for_the_population_rows_mentions_the_cost_and_the_key() {
        ...
        assert!(setting_description(30).contains("濃さ"));
        assert!(setting_description(31).contains("実績"), "2020年が実績であることに触れる");
    }

    #[test]
    fn settings_rows_expands_the_population_year_accordion_below_its_row() {
        let cfg = Config::default();
        let (_, its, sel) = settings_rows(&test_args(), &cfg, Some(31), false, 31, 2);
        assert_eq!(its.len(), SETTINGS_ROW_COUNT + crate::config::POPULATION_YEARS.len());
        assert_eq!(its[31], "▾ 人口の年次 2025年");
        assert_eq!(its[32], "    2020年(実績)");
        assert_eq!(its[33], "    2025年(推計)");
        assert_eq!(sel, 34); // 32(先頭候補) + set_pick_sel(2)
    }
}
