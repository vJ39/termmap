// JIS X 0410「地域メッシュ」の1次/2次メッシュ計算。プロットデータ(交通量・主要道路・通行規制)のディスクキャッシュのキーに使う。
// bboxを生のままキーにすると1pxパンで別キーになりヒットしないため、固定のメッシュへ丸める(docs/plot-data-disk-cache-design.md §4)。
//   1次メッシュ … 緯度40分(=2/3度)×経度1度。コード4桁 = p*100+u (p = floor(lat*1.5) / u = floor(lon)-100)
//   2次メッシュ … 1次を8×8に分割(緯度1/12度 × 経度1/8度)。コード6桁 = 1次コード*100 + row*10 + col (row=南から0..7 / col=西から0..7)
// std のみに依存する(単体テストで完結できるようにするため)。
...
/// primary_codes と同じ列挙だが、MAX_CODES の安全弁を持たない。過去災害/境界は plotlayer::PlotLayer 側でジョブの
/// セル数上限を外しているので(docs/disaster-choropleth-unlimited-zoom-design.md §3.1)、ここでも黙って0件を返さない。
/// 日本のメッシュ空間(p,uが0..=99)の外は除外するので、返る件数は最大10,000(100×100)。
...
// ---- 2分の1地域メッシュ(500mメッシュ・9桁。docs/population-mesh-overlay-design.md §2.4) ----
// 3次メッシュ(約1km)を4分割したもの。国土数値情報の500mメッシュ別将来推計人口が使う単位。
//   MESH_ID = p p u u  r c  r3 c3  q      例: 523351132
//             └1次┘   └2次┘└3次┘ └500m┘
//   q: 1=南西 2=南東 3=北西 4=北東 / 高さ 1/240 度(緯度15秒)・幅 1/160 度(経度22.5秒)
// 下の2関数はどちらも、緯度1/240度・経度1/160度で刻んだ整数格子の番号を経由して計算する。
