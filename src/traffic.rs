// 道路交通量(JARTIC「交通量オープンデータ」、WFS・登録不要)。直轄国道の常時観測点の実測台数
// (5分値)を「混雑度の目安」として重ねる。事故情報・区間旅行時間・交通規制情報は含まれない。
// 時間範囲を絞らずに叩くとバックエンドがOOMで落ちる(要CQL_FILTER)。道路種別は"3"(一般国道)のみ。
...
mod tests {
    ...
    // 実際のtargetTimes/GetFeature応答の抜粋(実測、3件)。
