...
// 設計は docs/population-mesh-overlay-design.md §4.2。47都道府県の500mメッシュ人口を直列に取得し、
// MESH_ID と SHICODE から2次メッシュと都道府県の組を集める。県境を跨ぐ2次メッシュは複数の都道府県を
// 持つので、取りこぼして県境の帯が欠けないよう索引は過剰側に倒す。再生成は推計の版が上がったときだけ。
// 配信元への負荷を考えて並列化はしない。本体とはモジュールを共有できない(別クレートになる)ので、
// 必要な2ファイルだけを #[path] で取り込む(population.rs の crate:: 参照が mesh だけなのはこのため)。
