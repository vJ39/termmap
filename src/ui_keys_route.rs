// ルート(経由地)まわりの画面のキー処理(ui_keys.rs の Focus 分岐から切り出した1つ)。おまかせ周回の
// 距離ゲージ・ルート名の保存・お気に入りルート一覧・道路の塊一覧・経由地の並べ替えビュー・ルートパネル。
// 経由地を触る分岐はどれも trigger_route() を呼び直す(2点以上あれば別スレッドで経路計算が始まる)。
// 引数は依存が分かるよう各画面が実際に使う値だけにする(3つ以上必要な画面は KeyCtx をまとめて受け取る)。
...
mod tests {
    ...
    // ui_keys::dispatch は focus を Map へ倒してから呼ぶので、テストも同じ前提で始める(「画面を出した
    // ままにする」分岐だけが focus を書き戻す)。trigger_route() で BRouter を呼ぶスレッドを起こす分岐
    // (経由地2点以上での掴んだままの上下・[ ]の入れ替え)と、$HOME/.config/termmap/routes/ を書く
    // ルート名の保存は触らない。確かめるのは入力・カーソル移動・画面遷移・並べ替え/削除の結果だけ。
    fn base() -> UiState {
