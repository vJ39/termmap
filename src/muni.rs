// 市区町村の境界(気象庁 class20s の区域ポリゴン)。過去災害コロプレスの下地(設計は
// docs/disaster-choropleth-design.md §2)。ファイルは0始まりの10枚で、_0(北海道)を取りこぼすと道内が
// 丸ごと欠ける。座標は JMA 側で簡略化済みなので間引かない。リングの巻き方向が不統一なので even-odd で
// 塗る(geopoly.rs)。crate:: は純粋な幾何の geopoly だけを参照し、単体テストはネットワーク無しで完結する。
...
/// `https://www.jma.go.jp/bosai/common/const/relm.json` の写し。`RELM[i]` が `class20s_{i}.json` の
/// 中身の外接矩形で、並びは (lat_min, lon_min, lat_max, lon_max)(plotlayer::Bbox と同じ)。
/// 10要素の小さな配列で内容が変わらないので、取りに行かず定数で持つ。矩形どうしは重なっているが、
/// 視野に入る区域は必ず、視野と交差する矩形のファイルのどれかに入っている。
...
/// 政令指定都市のうち、JMA が区単位で区域を持つ市(広島市・神戸市)の区コード → 市コード。
/// NIED は政令市を市単位のコードでしか持たないので、この表が無いと両市が塗られない。他の政令市は
/// 市まるごとか横浜市型(`1410011`/`1410012` のように下2桁だけが細分)なので `code[..5]` で合う。
/// JMA がほかの政令市も区単位へ変えた場合はテストでも検出できず、その市が塗られなくなる。
...
impl MuniArea {
    /// NIED の CHIDAN_CODE と突き合わせる5桁のコード。7桁の先頭5桁を採り、広島市・神戸市の区だけ
    /// WARD_TO_CITY で市コードへ丸める。"hoppo" のような非数値もあるので整数へパースせず、5バイト未満・
    /// 文字境界でないコードは空文字を返す(塗る対象にならないだけ)。**ポリゴン自体は7桁のまま扱う**。
    /// 5桁へ統合すると区域どうしの内部境界にも縁取りが引かれるので、割り当ては5桁・描画は7桁に分ける。
...
mod tests {
    ...
    // 実応答の抜粋。利島村は class20s_3 の実データそのまま(9頂点・1ポリゴン)。横浜市北部はコードと
    // 名前だけ実在で、リングは多重ポリゴンが平らに並ぶことの確認用に4頂点×2ポリゴンへ間引いた。
    // 根室地方は code が数値でない実在の行(整数パースで落ちないことの確認用)。
    ...
    fn live_fetch_real_boundaries() {
        ...
        // 確認時点の実測値は 1,806区域・148,424頂点。合併等で多少動くので幅を持たせる。
