...
// 塗りは PTN_*(合算前)を使う(PT00_* は秘匿処理で0にされたメッシュが出る・§2.5)。crate:: 参照は mesh
// のみ。索引生成器(src/bin/gen-mesh2pref.rs)が #[path] でこのファイルと mesh.rs だけを取り込むため。
...
// ---- 2次メッシュ → 都道府県 の索引(§4.2) ----
// 都道府県の外接矩形では判定できない(東京都は小笠原・南鳥島まで含むため、沖縄本島を見ているだけで
// 東京(8.19MB)を取りに行ってしまう)。判定はメッシュコードで行い、索引はデータ自身から作る
// (各 feature が MESH_ID の上6桁=2次メッシュと SHICODE の上2桁=都道府県を両方持っている)。
