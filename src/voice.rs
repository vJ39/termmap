...
//! 残り距離が閾値を切ったタイミングで1回だけ読み上げ内容を返す。読み上げは sound.rs の Sound::play と
//! 同じく「常に両方試す」: ローカル実行時はmacOSの`say`、web実行時はブラウザのWeb Speech API(効果音と
//! 同じOSC経由の合図)。どちらの経路も、対応していない側では無害に無視される。
