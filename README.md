...
- **高速道路区間の色分け**: 確定ルートのうち高速道路を通る区間をルート線の上に緑で描き、どこからどこまで高速に乗るかを示す(常時表示・追加の問い合わせなし)。途中で高速を降りるルートでは要約に「高速32.3km(2区間)」のように区間数が出る
...
- **ルート沿い気象警報**: 設定画面で「ルート沿い気象警報」をONにすると、ルート確定のたびに通過する気象庁の一次細分区域を判定し、警報・注意報が出ている区間だけルート線を特別警報(紫)/警報(赤)/注意報(黄)で上塗りする(キー不要)
...
キー全体・Spaceメニューの構造・各画面の詳細操作は `docs/MANUAL.md` を参照。
    ...
    移動   ←↑↓→ パン(既定は細かく・押し続けで加速/Shift+矢印で常に高速) / hjkl 矢印と同じ(大文字HJKLで常に高速) / + - ズーム / マウス(ドラッグでパン・クリックで中心へ・ホイールでズーム) / Space メニュー
