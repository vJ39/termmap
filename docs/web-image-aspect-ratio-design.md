...
## 11. 実装後の変更: レターボックスからクロップ(cover方式)へ

§7.6で設計したレターボックス(contain、余白を足して全体を収める)方式は実装・実機確認の結果、
写真がセル矩形に対して大きく縮小され小さく見える影響が大きいと判断し、クロップ(cover、
セル矩形いっぱいに写真を敷き詰め、はみ出す方向は見切れさせる)方式へ変更した。

- `src/cellratio.rs`: `PhotoFit`(余白込みキャンバス+配置)を`PhotoCrop`(元画像内の切り出し矩形)へ、
  `fit_photo_into_cells`/`letterbox_photo`を`crop_photo_to_cells`/`crop_photo`へ置き換えた。
  黒帯を足す代わりに、写真から目的の縦横比の矩形を中央基準で切り出すだけなので、`MAX_CANVAS_PX`
  (縮小してもキャンバスが大きくなりすぎないようにするクランプ)は不要になり削除した。
- 方向の直感に注意: containでは「縦長端末→幅いっぱいに置いて上下に余白」だったが、
  coverでは軸が逆になり「縦長端末→**高さ**いっぱいを使って左右を切り落とす」になる
  (領域が縦長なほど、写真は左右を大きく失う)。横長端末も同様に軸が逆転する。
- `src/ui.rs`の実写・道路カメラ表示(旧 §7.6 の呼び出し箇所)を`cellratio::crop_photo`へ差し替えた。
