...
### 雨雲レーダー
気象庁の降水ナウキャストを地図に半透明で重ねる。出典: 気象庁ナウキャスト。

- `C` 雨雲レーダーの表示/非表示。ONにすると必ず最新の実況から始まる
- `>` 表示時刻を未来へ1コマ(5分)。予報は最大+60分先まで。OFFのときに押すとONにして最新の実況から始める
...
- おすすめ: `claude` CLI
- 雨雲レーダー: 気象庁 降水ナウキャスト(キー不要)。開発者向けAPIとして公開されたものではない非公式エンドポイントの個人利用であり、予告なく停止しうる。取得は表示中の1コマ分だけで、先読みはしない
