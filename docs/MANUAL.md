...
### Settings(設定画面。`,`)
- `↑` `↓`(または `w` `s`) 項目選択
- `Enter` / `Space` 選択項目を切替。ON/OFF項目はそのままトグル、選択肢が3つ以上ある項目(地図種別/既定ルート/提案AIモデル/画像解像度/中心十字の色)はその場でアコーディオン展開(項目の直下に候補がインデント表示され、他の項目は押し下げられる)
...
項目: 点字ドット / 地物色分け / 輪郭抽出 / 単色 / 地図種別(osm/voyager/dark/light/topo・一覧選択) / 既定ルート(下道/高速/最短・一覧選択) / 道路の点間隔 / スポット既定表示 / おすすめON-OFF / 提案AIモデル(sonnet/haiku/opus・一覧選択) / 実写ON-OFF / 画像表示ON-OFF / 画像解像度(高/中/低・一覧選択) / 移動中の低解像度化ON-OFF / サウンドON-OFF / オンボーディングON-OFF / 中心十字の色(一覧選択) / Google APIキー
