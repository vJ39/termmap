...
#   - </body> の直前に web/touch-overlay.js を <script> として埋め込む
# という2箇所だけを足したものを web/index.html として書き出す。
#
...
OUT="$OUT" BASE="$BASE" OVERLAY="$OVERLAY" VIEWPORT="$VIEWPORT" python3 - <<'PY'
import os, sys
...
# 現状は含まれないが、将来 touch-overlay.js に文字列として入っても壊れないようにしておく。
...
block = ('<script id="termmap-touch-overlay">\n' + overlay + '\n</script>')
...
grep -q 'termmap-touch-overlay' "$OUT" || { echo "error: 埋め込みの検証に失敗" >&2; exit 1; }
